    ...
    function backgroundTransition(){
        document.querySelectorAll(".scroll-container").forEach(function(scrollContainer){
            ...
            if(background.children.length > 1){
                ...
                /*
                function onForegroundExit(exit) {
                    exit.forEach((change) => {
                ...
                // grab all background elements
                background.childNodes.forEach(function(child){
                    if (child.nodeName != "#text" && child.nodeName != "#comment"){
                        backgroundItems.push(child);
                    }
                });

                // on exit of foreground item, iterate background
                let observer = new IntersectionObserver(onForegroundExit);

                for(let elm of foregroundItems) {
